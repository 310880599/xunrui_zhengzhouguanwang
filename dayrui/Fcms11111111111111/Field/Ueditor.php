...
class Ueditor extends \Phpcmf\Library\A_Field {
    ...
    public function input($field, $value = '') {
        ...
        $tool = IS_DEV || IS_ADMIN ? "'fullscreen', 'source', '|', " : ''; // 后台引用时显示html工具栏
         ...
         function dr_is_auto_description_".$field['fieldname']."() {
            ...
            $(function(){
                ...
                var editor_{$name} = new baidu.editor.ui.Editor({
                    ...
                    serverUrl:\"/index.php?s=api&c=file&token=".dr_get_csrf_token()."&m=ueditor&image_endstr=".trim((string)$field['setting']['option']['image_endstr'])."&image_reduce=".intval($field['setting']['option']['image_reduce'])."&attachment=".intval($field['setting']['option']['attachment'])."&is_wm=".$field['setting']['option']['watermark']."&rid=".$this->rid."&\",
                    lang: \"lang\",
