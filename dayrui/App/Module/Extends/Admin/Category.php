...
class Category extends \Phpcmf\Table {
    ...
    protected function _Extend_Init() {
        ...
        $menu = [
            ...
            '批量' => [APP_DIR.'/category/all_add', 'fa fa-plus'],
    ...
    protected function _get_tree_list($data) {
        ...
        if ($this->module['share'] && dr_is_app('chtml') && dr_in_array('html', $this->cat_config[$this->module['dirname']]['sys_field'])) {
        ...
        foreach($data as $k => $t) {
            ...
            $t['pcatpost'] = 0;
            $pid = explode(',', $t['pids']);
            $this->module['category'][$t['id']]['topid'] = $t['topid'] = isset($pid[1]) ? $pid[1] : $t['id'];
            if ($this->module['share']) {
                ...
                $t['setting']['chtml'] = intval($this->module['html']);
            ...
            if ($this->_is_admin_auth('edit') && dr_count($this->module['category_field']) && ($t['tid'] !=2 && $this->is_scategory)) {
            ...
            // 只对超管有效
            if (isset($this->admin['role'][1]) && dr_count($this->init['field'])) {
    ...
    public function config_add() {
        ...
        if (IS_POST) {
            ...
            // 自动更新缓存
            $this->_auto_cache(APP_DIR ? APP_DIR : 'share');
    ...
    protected function _Admin_List() {
        ...
        \Phpcmf\Service::V()->assign([
            ...
            'MAX_CATEGORY' => \Phpcmf\Service::M()->table($this->init['table'])->counts(),
        ]);
    ...
    protected function _Admin_Add() {
        ...
        if ($pid) {
            if (!$this->module['category'][$pid]) {
                $this->_admin_msg(0, dr_lang('父栏目【%s】缓存不存在', $pid));
            }
    ...
    protected function _Admin_All_Add() {
        ...
        if (IS_AJAX_POST) {
            ...
            foreach ($list as $t) {
                ...
                $data['ismain'] = 1;
                //if ($pid) {
                    //$data['ismain'] = (int)$this->module['category'][$pid]['ismain'];
                //}
                $data['disabled'] = 0;
                $data['pids'] = '';
                $data['thumb'] = '';
    ...
    public function field_edit() {
        ...
        \Phpcmf\Service::V()->assign([
            'field' => $this->module['category_field'],
            'select' => \Phpcmf\Service::L('tree')->ismain(true)->select_category(
                $this->module['category'],
                0,
                'id=\'dr_catid\' name=\'catid[]\' multiple="multiple"  data-actions-box="true"',
                '',
    ...
    protected function _Data($id = 0) {
        ...
        if ($row['ismain']) {
            ...
            if ($this->module['category'][$id]['setting']['cat_field']) {
                foreach ($this->module['category'][$id]['setting']['cat_field'] as $key => $v) {
                    unset($this->field[$key]);
                }
            }
        }
        ...
        return parent::_Save($id, $data, $old,
            function ($id, $data, $old){
                ...
                if ($pid) {
                    ...
                    //$save['ismain'] = (int)$this->module['category'][$save['pid']]['ismain'];
                }
    ...
    private function _auto_cache($cdir) {
