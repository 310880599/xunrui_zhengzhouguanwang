...
class Module extends \Phpcmf\Model {
    ...
    public function update_category_cache($siteid, $cdir) {
        ...
        $category = \Phpcmf\Service::M()->db->table($siteid.'_'.$cdir.'_category')
            ->where('disabled', 0)->orderBy('displayorder ASC, id ASC')->limit(MAX_CATEGORY)->get()->getResultArray();
    ...
    public function cache($siteid = SITE_ID, $module = null) {
        ...
        if (is_file(IS_USE_MODULE.'Models/Repair.php')) {
            unlink(IS_USE_MODULE.'Models/Repair.php');
        }
       ...
       if ($siteid == 1) {
        ...
        if (!$this->table('admin_menu')->where('uri', 'module/module/index')->counts()) {
            // 删除高级菜单
            $this->table('admin_menu')->where('mark', 'config-content')->delete();
            $this->table('admin_menu')->where('mark', 'config-seo')->delete();
            $this->table('admin_menu')->where('mark', 'content-verify')->delete();
            unlink(WRITEPATH.'is_module_install.lock');
        } else {
            $this->table('admin_menu')->where('mark', 'config-study-1')->delete();
            $this->table('admin_menu')->where('mark', 'config-study-2')->delete();
            $this->table('admin_menu')->where('mark', 'config-study-3')->delete();
        }
