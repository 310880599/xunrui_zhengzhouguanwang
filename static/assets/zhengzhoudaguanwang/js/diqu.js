...
function province() {
    ...
    //
    $("#txtprovince").val(provinceName[0]);
    cityName(e.options.selectedIndex);
   
}
function cityName(n) {
n=n+1;
 $("#txtprovince").val($("#province").val());
    var e = getObj('city');
...
function citychange(n){
 $("#txtcity").val(n);
}
function onloadprovince() {
    province(); //初始时给省名下拉菜单赋内容  
    if($("#txtprovince").val()!=""){
    $("#province").val($("#txtprovince").val()).change();}else{
     $("#province").get(0).selectedIndex=0;
    }
    if($("#txtcity").val()!=""){
    $("#city").val($("#txtcity").val());}else{
    $("#city").get(0).selectedIndex=0;
    }
} 
