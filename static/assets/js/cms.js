...
function dr_isEllipsis(e) {
	...
	return r.style.width = e.offsetWidth + "px", r.style.height = e.offsetHeight + "px", r.style.overflow = "auto", r
		.style.position = "absolute", r.style.zIndex = -1, r.style.opacity = 0, r.style.whiteSpace = "nowrap", r
		.innerHTML = e.innerHTML, (a = e.parentNode).appendChild(r), t = r.scrollWidth > r.offsetWidth, a.removeChild(
		r), t
}
...
function dr_is_mobile() {
	var e = navigator.userAgent,
		...
		r = /(?:Android)/.test(e),
		i = /(?:Firefox)/.test(e),
		o = (/(?:Chrome|CriOS)/.test(e), /(?:iPad|PlayBook)/.test(e) || r && !/(?:Mobile)/.test(e) || i && /(?:Tablet)/
			.test(e));
...
function dr_ftable_mydelete(e) {
	var a = $(e);
	a.parent().find(".form-control-file").val("0"), a.parent().find(".form-control-link").val("0"), a.parent().find(
			".form-control-preview").val(""), a.parent().find(".ftable-show").hide(), a.parent().find(".ftable-delete")
		.hide()
}
			...
			return $.ajax({
				...
				success: function(a) {
					...
					if (1 == a.code) {
						...
						t.parent().find(".form-control2").val(n.id), t.parent().find(
								".form-control-link").val(n.url), t.parent().find(
								".form-control-preview").val(n.url), t.parent().find(
								".ftable-show").show(), t.parent().find(".ftable-delete")
...
function dr_ftable_myshow(e) {
	...
	var o = "";
	"undefined" != typeof is_cms_dev && is_cms_dev && (o = '<p style="text-align: center"><a href="' + r +
		'" target="_blank">' + r + "</a></p>"), top.layer.alert(o + '<p style="text-align: center"><a href="' + t +
		'" target="_blank"><img style="max-width:100%" src="' + r + "?" + Date.parse(new Date) + '"></a></p>', {
			shade: 0,
			shadeClose: !0,
			title: "",
			area: [i, i],
...
function dr_ftable_textareainput(e) {
	...
	1 == is_mobile_cms && (t = r = "90%"), layer.open({
		...
		content: '<div style="padding:10px;height:100%;"><textarea style="height:100%;" id="form-' + e +
			'" class="form-control">' + a + "</textarea></div>"
	})
}
...
function dr_preview_video(e) {
	...
	var r = "";
	"undefined" != typeof is_cms_dev && is_cms_dev && (r = '<p style="text-align: center"><a href="' + e +
		'" target="_blank">' + e + "</a></p>"), layer.alert(r +
		'<p style="text-align: center"> <video class="video-js vjs-default-skin" controls="" preload="auto" ' + t +
		'><source src="' + e + '" type="video/mp4"/></video>\n</p>', {
			shade: 0,
			shadeClose: !0,
			title: "",
			area: [a, a],
...
function dr_preview_image(e) {
	...
	var t = "";
	"undefined" != typeof is_cms_dev && is_cms_dev && (t = '<p style="text-align: center"><a href="' + e +
		'" target="_blank">' + e + "</a></p>"), top.layer.alert(t + '<p style="text-align: center"><a href="' + e +
		'" target="_blank"><img style="max-width:100%" src="' + e + "?" + Date.parse(new Date) + '"></a></p>', {
			shade: 0,
			shadeClose: !0,
			title: "",
			area: [a, a],
...
function dr_preview_url(e) {
	var a = "400px";
	1 == is_mobile_cms && (a = "90%"), layer.alert('<div style="text-align: center;"><a href="' + e +
...
function dr_iframe(type, url, width, height, rt) {
	var title = "";
	title = "add" == type ? '<i class="fa fa-plus"></i> ' + dr_lang("添加") : "edit" == type ?
		'<i class="fa fa-edit"></i> ' + dr_lang("修改") : "send" == type ? '<i class="fa fa-send"></i> ' + dr_lang("推送") :
		"save" == type ? '<i class="fa fa-save"></i> ' + dr_lang("保存") : type, width || (width = "500px"), height || (
			height = "70%"), 1 == is_mobile_cms && (width = "95%", height = "90%"), layer.open({
			type: 2,
			title: title,
			fix: !0,
			scrollbar: !1,
				...
				return $.ajax({
					...
					success: function(json) {
						if (layer.close(loading), json.token) {
							...
							$(body).find("#myform input[name='" + token.name + "']").val(token
								.value)
						}
						if (json.code) {
							if (layer.close(index), json.data.jscode) return void eval(json.data
								.jscode);
							json.data.tourl ? setTimeout("window.location.href = '" + json.data
									.tourl + "'", 2e3) : "nogo" == rt || setTimeout(
									"window.location.reload(true)", 2e3), json.data.htmlfile && $
								.ajax({
									type: "GET",
						...
						} else $(body).find("#dr_row_" + json.data.field).addClass("has-error"),
...
function dr_iframe_show(e, a, t, r, i) {
	var o = "";
	o = "show" == e ? '<i class="fa fa-search"></i> ' + dr_lang("查看") : "edit" == e ? '<i class="fa fa-edit"></i> ' +
		dr_lang("修改") : "code" == e ? '<i class="fa fa-code"></i> ' + dr_lang("代码") : "cart" == e ?
		'<i class="fa fa-shopping-cart"></i> ' + dr_lang("交易记录") : e, t || (t = "60%"), r || (r = "70%"), 1 ==
		is_mobile_cms && (t = "95%", r = "90%"), layer.open({
			type: 2,
			title: o,
			fix: !0,
			maxmin: !0,
		...
		$.ajax({
			...
			success: function(json) {
				if (layer.close(loading), json.code) {
					...
					json.data.url ? setTimeout("window.location.href = '" + json.data.url + "'",
						2e3) : tourl && setTimeout("window.location.href = '" + tourl + "'",
						2e3)
				}
...
function dr_ajax_url(e) {
	...
	$.ajax({
		...
		dataType: "json",
		success: function(e) {
			layer.close(a), 0 == e.code && ($(".fc-code img").click(), e.data.field && ($("#dr_row_" + e
				.data.field).addClass("has-error"), $("#dr_" + e.data.field).focus())), dr_cmf_tips(e
				.code, e.msg, e.data.time), e.data.url && setTimeout("window.location.href = '" + e.data
				.url + "'", 2e3)
		},
		error: function(e, a, t) {
			dr_ajax_alert_error(e, this, t)
...
function dr_ajaxp_url(e) {
	...
	$.ajax({
		...
		dataType: "jsonp",
		success: function(e) {
			layer.close(a), dr_cmf_tips(e.code, e.msg, e.data.time), e.data.url && setTimeout(
				"window.location.href = '" + e.data.url + "'", 2e3)
		},
		error: function(e, a, t) {
			dr_ajax_alert_error(e, this, t)
		}
		...
		$.ajax({
			...
			success: function(json) {
				...
				if (json.code) {
					...
					json.data.url ? setTimeout("window.location.href = '" + json.data.url + "'",
						2e3) : setTimeout("window.location.reload(true)", 3e3)
				}
	...
	layer.confirm(a, {
		...
		$.ajax({
			...
			success: function(e) {
				...
				e.code && (e.data.url ? setTimeout("window.location.href = '" + e.data.url + "'",
					2e3) : setTimeout("window.location.href = '" + t + "'", 2e3)), dr_cmf_tips(e
					.code, e.msg, e.data.time)
			},
			error: function(e, a, t) {
				dr_ajax_alert_error(e, this, t)
			}
...
function dr_ajax_submit(e, a, t, r) {
	...
	if (0 != i)
		if (i > 1) dr_cmf_tips(0, dr_lang("表单id属性已重复定义") + " (" + a + ")");
			...
			"required" == o.val() && o.val(""), 1 == $("#" + a).find("[name=is_admin]").val() && $("#" + a).find(
				".dr_required").each(function() {
			...
			success: function(i) {
				...
				if (i.code) {
					if (dr_cmf_tips(1, i.msg, i.data.time), i.data.htmlfile && $.ajax({
						...
						var n = e;
						"" != r && null != r && "undefined" != r ? n = r : i.data.url && (n = i.data.url),
					...
					}) : dr_cmf_tips(0, i.msg, i.data.time), $("#" + a + " .fc-code img").click(), i.data
					.field && ($("#" + a + " #dr_row_" + i.data.field).addClass("has-error"), $("#" + a +
						" #dr_" + i.data.field).focus())
			},
			error: function(e, a, t) {
				dr_ajax_alert_error(e, this, t)
			}
...
function dr_loginout(e) {
	$.ajax({
		...
		success: function(a) {
			...
			dr_cmf_tips(1, a.msg, a.data.time), e && "退出成功" != e ? setTimeout("window.location.href = '" +
				e + "'", 2e3) : setTimeout('window.location.href="' + a.data.url + '"', 2e3)
		},
		error: function(e, a, t) {
			dr_ajax_alert_error(e, this, t)
		}
			...
			$("#" + a + " .form-group").removeClass("has-error"), $.ajax({
				...
				success: function(e) {
					...
					if (e.code) {
						...
						void 0 !== t && t && t.length > 2 ? (dr_cmf_tips(1, e.msg, e.data.time), setTimeout(
							'window.location.href="' + t + '"', 3e3)) : e.data.url && (o.length > 2 ? (
								dr_cmf_tips(1, e.msg, e.data.time), setTimeout(
									'window.location.href="' + e.data.url + '"', 3e3)) : window.location
							.href = e.data.url)
					} else dr_cmf_tips(0, e.msg, e.data.time), $(".fc-code img").click(), e.data.field && (
...
function dr_module_digg(e, a, t) {
	$.get(dr_get_web_dir() + "index.php?is_ajax=1&s=api&app=" + e + "&c=module&m=digg&id=" + a + "&value=" + t,
		function(e) {
...
function check_title(e) {
	var a = $("#dr_id").val(),
		t = $("#dr_title").val(),
		r = $("#dr_module").val();
	$.get(dr_get_web_dir() + "index.php?s=api&c=api&m=checktitle&title=" + t + "&module=" + r + "&id=" + a +
		"&is_ajax=1",
		function(e) {
			e && dr_cmf_tips(0, e)
		})
...
function get_keywords(e) {
	var a = $("#dr_title").val(),
		t = $("#dr_module").val();
	if ($("#dr_" + e).val()) return !1;
	$.get(dr_get_web_dir() + "index.php?s=api&c=api&m=getkeywords&title=" + a + "&module=" + t + "&is_ajax=1", function(
		a) {
...
function d_isurl(e) {
	var a = $("#dr_" + e).val();
	return 1 == new RegExp(/http(s)?:\/\/([\w-]+\.)+[\w-]+(\/[\w- .\/?%&=]*)?/).test(a) ? (d_tips(e, !0), !1) : (d_tips(
		e, !1), !0)
}
...
function dr_ajax_alert_error(e, a, t) {
	if (layer.closeAll("loading"), "undefined" != typeof is_admin && is_admin) {
		...
		i += "请求方式：" + a.type + "<br>", i += "请求地址：" + a.url + "<br>", r || (r = t), "1" == is_admin ? layer.open({
			...
			content: '<div style="padding:10px;border-bottom: 1px solid #eee;">' + i +
				'</div><div style="padding:10px;">' + r + "</div>"
		}) : layer.open({
			type: 1,
			title: dr_lang("系统崩溃，请检查错误日志"),
			fix: !0,
...
function dr_slimScroll_init(e, a) {
	if ($().slimScroll) {
		...
		if ("1" === $(r).attr("data-inited")) {
			...
			var o = {};
			$(r).attr("data-handle-color") && (o["data-handle-color"] = $(r).attr("data-handle-color")), $(r).attr(
				"data-wrapper-class") && (o["data-wrapper-class"] = $(r).attr("data-wrapper-class")), $(r).attr(
				"data-rail-color") && (o["data-rail-color"] = $(r).attr("data-rail-color")), $(r).attr(
				"data-always-visible") && (o["data-always-visible"] = $(r).attr("data-always-visible")), $(r).attr(
				"data-rail-visible") && (o["data-rail-visible"] = $(r).attr("data-rail-visible")), $(r).slimScroll({
				wrapperClass: $(r).attr("data-wrapper-class") ? $(r).attr("data-wrapper-class") :
					"slimScrollDiv",
				destroy: !0
			});
			var n = $(r);
...
$(function() {
		($(document).width() < 600 ? ($(".hidden-mobile").hide(), $(".table").attr("style",
			"table-layout: inherit!important;")) : $(".hidden-mobile").show(), $(".table .heading th").click(
			function(e) {
			...
			jQuery(e).each(function() {
				a ? ($(this).prop("checked", !0), $(this).parents("tr").addClass("active")) : ($(
	...
	function(e, a) {
		...
		a.exports = function(e) {
			...
			return new Promise(function(t, s) {
				var c = a.jsonpCallbackFunction || "jsonp_" + Date.now() + "_" + Math.ceil(1e5 * Math
						.random()),
					f = d + "_" + c;
				window[c] = function(e) {
					t({
				...
				_.setAttribute("src", "" + o + d + "=" + c), a.charset && _.setAttribute("charset", a
						.charset), _.id = f, document.getElementsByTagName("head")[0].appendChild(_),
					l = setTimeout(function() {
						s(new Error("JSONP request to " + e + " timed out")), r(c), i(f), window[
							c] = function() {
								r(c)
							}
					}, n), _.onerror = function() {
						s(new Error("JSONP request to " + e + " failed")), r(c), i(f), l &&
							clearTimeout(l)
					}
			})
		}
